...
| Location | Contents |
|---|---|
| `adata.obs` | Per-cell metadata: `Image`, `Object ID`, `Xµm`, `Yµm`, `<marker>_pos` boolean columns, `annotation__<label>` boolean columns |
...
- **Manual threshold TSV/CSV** *(fallback)* — if no usable single-measurement classifier JSONs are found, QXYCell looks for a filled manual threshold table such as `manual_thresholds.tsv`, `marker_thresholds.tsv`, or `qxycell_thresholds.tsv`.
...
```text
qxy_outputs_YYMMDD-HHMM/tables/manual_threshold_template.tsv
```
...
a name such as `manual_thresholds.tsv`, then rerun `qxy.check()` or
`qxy.run()`.

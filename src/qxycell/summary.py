"""Descriptive summary reports for QXYCell AnnData objects."""
...
def dataset_summary(
    ...
    *,
    ...
    """Write descriptive summary tables for a QXYCell AnnData object."""

    import pandas as pd

    out_dir = resolve_output_dir(output_dir, adata=adata) / "dataset_summary"
    ...
    ignore_cols = [column for column in annotation_cols if "ignore" in str(column).lower()]
    ...
    paths["overview"] = _write_table(overview, out_dir / "dataset_overview.tsv")

    if sample_col in obs.columns:
        cells_per_sample = sample_values.value_counts(dropna=False).rename_axis(sample_col).reset_index(name="n_cells").sort_values(sample_col)
    ...
    if celltype_col in obs.columns:
        celltype_counts = obs[celltype_col].astype(str).value_counts(dropna=False).rename_axis(celltype_col).reset_index(name="n_cells")
        ...
        if sample_col in obs.columns:
            by_sample = obs.assign(**{sample_col: obs[sample_col].astype(str), celltype_col: obs[celltype_col].astype(str)}).groupby([sample_col, celltype_col], observed=True).size().rename("n_cells").reset_index()
    ...
    if marker_cols:
        marker_table = pd.DataFrame([
            {"marker": str(column)[:-len(marker_suffix)], "column": column, "n_positive": int(obs[column].fillna(False).astype(bool).sum()), "fraction_positive": float(obs[column].fillna(False).astype(bool).mean())}
        ...
        if sample_col in obs.columns:
            ...
            for sample, indices in sample_values.groupby(sample_values).groups.items():
                ...
                for column in marker_cols:
                    ...
                    rows.append({sample_col: sample, "marker": str(column)[:-len(marker_suffix)], "column": column, "n_positive": int(values.sum()), "fraction_positive": float(values.mean()) if len(values) else 0.0})
            marker_by_sample = pd.DataFrame(rows)
            tables["marker_positivity_by_sample"] = marker_by_sample
            paths["marker_positivity_by_sample"] = _write_table(marker_by_sample, out_dir / "marker_positivity_by_sample.tsv")

    if annotation_cols:
        annotation_table = pd.DataFrame([
            {"annotation_column": column, "n_cells": int(obs[column].fillna(False).astype(bool).sum()), "fraction": float(obs[column].fillna(False).astype(bool).mean())}
    ...
    if ignore_cols:
        ...
        ignore_table = pd.DataFrame([{"ignore_columns": "; ".join(map(str, ignore_cols)), "n_ignore_cells": int(flagged.sum()), "fraction_ignore": float(flagged.mean())}])
        tables["ignore_cells"] = ignore_table
        paths["ignore_cells"] = _write_table(ignore_table, out_dir / "ignore_cells.tsv")

    html_parts = [
        '<html><head><meta charset="utf-8"><title>QXYCell Dataset Summary</title>',
        ...
        '</head><body><h1>QXYCell Dataset Summary</h1>',
    ]
    for name, table in tables.items():
        html_parts.append(f"<h2>{escape(name.replace('_', ' ').title())}</h2>")
        html_parts.append(table.to_html(index=False, escape=True))
    html_parts.append("</body></html>")
    html_path = out_dir / "dataset_summary.html"
    html_path.write_text("\n".join(html_parts), encoding="utf-8")
    paths["html"] = html_path

    result = {"output_dir": out_dir, "paths": paths, "tables": tables}
    adata.uns["qxycell_dataset_summary"] = {"output_dir": str(out_dir), "paths": {key: str(value) for key, value in paths.items()}}
    if verbose:
        print(f"Saved QXYCell dataset summary:\n{html_path}")
    return result
